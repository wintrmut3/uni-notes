...
for i = 1:length(v_r)
  for j = 1:length(v_t)
     % if only r or t are desired as constant input, 
     %(ex, final value for vector of rates but constant time of after 3 years) 
     %change v_r(i) or v_t(j) respectively to the desired constant value
